...
let show = function (tipStr,leftStr,rightStr,callback) {
    cc.loader.loadRes("Alert",cc.Prefab, function (error, prefab){
        ...
        cc.find("Alert/alertBackground/detailLabel").getComponent(cc.Label).string = tipStr;
        if(callback){
 
            cc.log("test2");
            cc.find("Alert/alertBackground/enterButton").on('click', function (event) {
                cc.log("test");
                dismiss();
                callback(leftStr);
            }, this);
 
            // cc.find("tipAlert/alertBackground/cancelButton/btn1").on(cc.Node.EventType.TOUCH_START, function (event) {
            //     dismiss();
            //     callback(rightStr);
            // }, this);
        }
